...
from catalog.validators.imports import UploadFileUpdateEditingStatusValidator
...
class UploadFileImportUpdateEditingStatusTestCase(APITestCase):
    ISSUE_KEY = 'SC-548'

    def setUp(self):
        out = io.BytesIO()
        df = pd.DataFrame(data=np.random.random((TITLE_ROW_OFFSET + 1, 20))).to_excel(out)
        out.seek(0)
    ...
    def run_validator(self):
        ...
        UploadFileUpdateEditingStatusValidator.validate({
            'files': files,
        })
    ...
    def test_passFileInvalidHeader__raiseBadRequestException(self):
        out = io.BytesIO()
        df = pd.DataFrame(data=np.random.random((1, 20))).to_excel(out)
        out.seek(0)
        self.file = FileStorage(
            stream=out,
            filename=fake.text(),
    ...
    def test_raiseBadRequestException__whenTooManyNewFile(self):
        for _ in range(MAX_IMPORT_FILE_PENDING + 1):
            fake.file_import(type='update_editing_status', status='new')
        with pytest.raises(exc.BadRequestException) as error_info:
            self.run_validator()
    ...
    def test_raiseBadRequestException__whenTooManyRow(self):
        out = io.BytesIO()
        df = pd.DataFrame(data=np.random.random((1001 + TITLE_ROW_OFFSET, 20))).to_excel(out)
        out.seek(0)
        self.file = FileStorage(
            stream=out,
            filename=fake.text(),
