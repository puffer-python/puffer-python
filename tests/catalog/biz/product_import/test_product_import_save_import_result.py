...
from catalog.biz.result_import import capture_import_result, CreateProductImportCapture, CreateProductImportSaver, \
    ImportStatus
...
class SaveImportResultTestCase(unittest.TestCase, JiraTest):
    ...
    def run_capture_result(self, capture_object):
        ...
        saver = CreateProductImportSaver(
            ...
            status=capture_object.status or random.choice([ImportStatus.FAILURE,ImportStatus.SUCCESS,ImportStatus.FATAL]),
    ...
    def test_save_result_check_updated_at(self, mock_object):
        ...
        file_import = fake.file_import(
            ...
            path=os.path.join(config.ROOT_DIR,
                              "tests/storage/template/template_create_DON.xlsx"),
            set_id=None
        )
    ...
    @patch('catalog.biz.result_import.CreateProductImportCapture._call_job', return_value=None)
    def test_save_result_check_updated_at_upload_2_times(self, mock_object):
        attribute_set = self.fake_attribute_set(is_variation=0, name='Sữa')
        self.fake_uom(attribute_set)
        ...
        with logged_in_user(self.user):
            ...
            import_capture = CreateProductImportCapture(attribute_set_id=attribute_set.id, import_id=file_import_id,
                                                        importer=fake.result_import_row(type='DON'))
            self.run_capture_result(import_capture)
            file_import_1 = fake.file_import(
                user_info=self.user,
                type='create_product_basic_info',
                status='new',
                path=os.path.join(config.ROOT_DIR,
            ...
            file_import_id_1 = file_import_1.id
            import_capture = CreateProductImportCapture(attribute_set_id=attribute_set.id, import_id=file_import_id_1,
                                                        importer=fake.result_import_row(type='DON'))
            self.run_capture_result(import_capture)

        result_imports = models.ResultImport.query.filter().all()
        assert len(result_imports) == 2
        assert result_imports[-1].updated_at.date() == datetime.datetime.now().date()
    ...
    @patch('catalog.biz.result_import.CreateProductImportCapture._call_job', return_value=None)
    def test_save_result_with_rows_cha_and_con(self, mock_object):
        attribute_set = self.fake_attribute_set(is_variation=0, name='Sữa')
        self.fake_uom(attribute_set)
        ...
        with logged_in_user(self.user):
            import_capture = CreateProductImportCapture(attribute_set_id=attribute_set.id, import_id=file_import_id,
                                                        importer=fake.result_import_row(type='CHA'))
            self.run_capture_result(import_capture)
            import_capture_con = CreateProductImportCapture(attribute_set_id=attribute_set.id, import_id=file_import_id,
                                                        importer=fake.result_import_row(type='CON'))
            self.run_capture_result(import_capture_con)


        result_imports = models.ResultImport.query.filter().all()
        assert len(result_imports) == 2
        assert result_imports[0].data.get('type') == 'CHA'
        assert result_imports[1].data.get('type') == 'CON'


    @patch('catalog.biz.result_import.CreateProductImportCapture._call_job', return_value=None)
    def test_save_result_with_rows_cha_fail_con_fail(self, mock_object):
        attribute_set = self.fake_attribute_set(is_variation=0, name='Sữa')
        self.fake_uom(attribute_set)
        ...
        with logged_in_user(self.user):
            import_capture = CreateProductImportCapture(attribute_set_id=attribute_set.id, import_id=file_import_id,
                                                        importer=fake.result_import_row(type='CHA'))
            import_capture.status = ImportStatus.FAILURE
            self.run_capture_result(import_capture)
            import_capture_con = CreateProductImportCapture(attribute_set_id=attribute_set.id, import_id=file_import_id,
                                                            importer=fake.result_import_row(type='CON'))
            import_capture_con.status = ImportStatus.FAILURE
            self.run_capture_result(import_capture_con)

        result_imports = models.ResultImport.query.filter().all()
        assert len(result_imports) == 2
        assert result_imports[0].data.get('type') == 'CHA'
        assert result_imports[0].status == ImportStatus.FAILURE
        assert result_imports[1].data.get('type') == 'CON'
        assert result_imports[1].status == ImportStatus.FAILURE

    @patch('catalog.biz.result_import.CreateProductImportCapture._call_job', return_value=None)
    def test_save_result_with_rows_cha_success_con_fail(self, mock_object):
        attribute_set = self.fake_attribute_set(is_variation=0, name='Sữa')
        self.fake_uom(attribute_set)
        ...
        with logged_in_user(self.user):
            import_capture = CreateProductImportCapture(attribute_set_id=attribute_set.id, import_id=file_import_id,
                                                        importer=fake.result_import_row(type='CHA'))
            import_capture.status = ImportStatus.SUCCESS
            self.run_capture_result(import_capture)
            import_capture_con = CreateProductImportCapture(attribute_set_id=attribute_set.id, import_id=file_import_id,
                                                            importer=fake.result_import_row(type='CON'))
            import_capture_con.status = ImportStatus.FAILURE
        ...
        assert len(result_imports) == 2
        assert result_imports[0].data.get('type') == 'CHA'
        assert result_imports[0].status == ImportStatus.SUCCESS
        assert result_imports[1].data.get('type') == 'CON'
        assert result_imports[1].status == ImportStatus.FAILURE

    @patch('catalog.biz.result_import.CreateProductImportCapture._call_job', return_value=None)
    def test_save_result_with_sp_DON_fail(self, mock_object):
        attribute_set = self.fake_attribute_set(is_variation=0, name='Sữa')
        self.fake_uom(attribute_set)
    ...
    @patch('catalog.biz.result_import.CreateProductImportCapture._call_job', return_value=None)
    def test_save_result_with_sp_DON_success(self, mock_object):
        attribute_set = self.fake_attribute_set(is_variation=0, name='Sữa')
        self.fake_uom(attribute_set)
